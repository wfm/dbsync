...
class Comparison:
    ...
    def output_table(self, table: IM.Table) -> None:
        ...
        if self.is_dst_table(table.name):
            # this is not a src table
            return

        src_inserts = self.repo.get_inserts(table.name)
        dst_name = self.get_dst_name(table.name)
    ...
    def write_high_water_marks(self):
        ...
        with open(hw_filename, mode="w", encoding="utf-8") as stream:
            ...
            high_water_marks: Dict[str, List[int]] = {}
            for _, table in self.repo.tables.items():
                if not self.is_dst_table(table.name):
                    dst_name = self.get_dst_name(table.name)
                    dst_table = self.repo.get_table(dst_name)
                    base_name = Settings.obj().get_base_table_name(table.name)
                    high_water_marks[base_name] = \
                        [table.get_autoinc_val(), dst_table.get_autoinc_val()]
